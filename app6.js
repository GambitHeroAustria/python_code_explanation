...
function soloGuesses(){
  const opts=SOLO_GUESS_OPTIONS[R]||SOLO_GUESS_OPTIONS.SOLOW1,g={},meta=soloWineMeta();
...
async function soloReveal(){
  clearPoll();
  const r=await rpc('solo_get_reveal',{p_code:R,p_token:getSession(R).token}),me=r.leaderboard.find(x=>x.name===getSession(R).name),meta=soloWineMeta()||soloImageByWineName(r.wine?.name||'');
  const d=r.wine?.details||{};
  A.innerHTML=brand(`<span class="room">Auflösung</span>`)+`<div class="toprow">${backLink()}</div>${meta?soloBottleFigure(meta,'reveal'):''}<section class="card hero revealhero"><h2>${me?`${e(me.name)}: ${me.score}/${me.max_score} Punkte`:'Auflösung'}</h2></section><section class="card"><h3>${e(r.wine.name)} ${e(r.wine.vintage||'')}</h3><p class="muted">${e(d.summary||d.sources_summary||'')}</p>${d.dossier?`<p>${e(d.dossier)}</p>`:''}${d.grapes?`<p><b>Rebsorte:</b> ${e(d.grapes)}</p>`:''}${d.region?`<p><b>Herkunft:</b> ${e(d.region)}</p>`:''}<p><b>Servieren:</b> ${e(d.serve||d.serving||'–')}</p></section><section class="card"><h3>Leaderboard</h3>${roomRanks(r.leaderboard)}</section>`;
}
